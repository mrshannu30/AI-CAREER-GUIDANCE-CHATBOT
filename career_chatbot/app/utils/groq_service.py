"""
Groq Integration for Career Chatbot
Free, unlimited API with fast inference
"""
import os
...
class CareerAdvisor:
    """Use Groq to provide intelligent career guidance"""
    
    def __init__(self):
        api_key = os.getenv('GROQ_API_KEY')
        if not api_key:
            raise ValueError("⚠️ GROQ_API_KEY not found in .env file. Please add it.")
...
def get_advisor():
    """Get or create career advisor instance"""
    global _advisor
    if _advisor is None:
        try:
            _advisor = CareerAdvisor()
        except ValueError as e:
            print(f"⚠️ Career Advisor initialization failed: {e}")
            return None
    return _advisor
